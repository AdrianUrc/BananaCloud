...
impl Security {
    ...
    /*pub async fn unban_ip(&self, ip: core::net::Ipv4Addr) {
        let mut locked_banned_ips = self.banned_ips.lock().await;
        let mut index_to_rm: Option<usize> = None;

        for (i, ip_addr) in locked_banned_ips.iter().enumerate() {
            if *ip_addr == ip {
                index_to_rm = Some(i);
                break;
            }
        ...
        if let Some(index) = index_to_rm {
            locked_banned_ips.remove(index);
            tracing::info!("[SECURITY] IP: {} was unbanned from server.", ip);
        } else {
            tracing::warn!("[WARNING] IP not found: {}", ip);
        }
    }*/
