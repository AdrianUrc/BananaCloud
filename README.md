...
4. Move on certs folder and generate SSL CA, certs and keys:
   ...
   5. Sign server's CSR with your CA: `openssl x509 -req -in server.csr -CA ca.pem -CAkey ca-key.pem -CAcreateserial \
  -out server-cert.pem -days 365 -sha256`
   ...
   8. Sign client's CSR with your CA: `openssl x509 -req -in client.csr -CA ca.pem -CAkey ca-key.pem -CAcreateserial \
...
5. Move 'ca.pem', 'server-cert.pem' and 'server-key.pem' to the project's home-path.
